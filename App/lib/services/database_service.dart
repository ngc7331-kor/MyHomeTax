...
class TaxTransaction {
  ...
  factory TaxTransaction.fromFirestore(DocumentSnapshot doc) {
    final data = doc.data() as Map<String, dynamic>;
    // 🛡️ Robust Field Name Check (v80)
    final String req = data['requester'] ?? data['name'] ?? data['recorder'] ?? '';
...
class DatabaseService {
  ...
  Stream<List<TaxTransaction>> getPendingApprovals({String? userId}) {
    ...
    // Note: userId 필터링 시 Firestore의 복합 인덱스가 필요할 수 있음
    // 하지만 부모용(userId=null)은 이미 인덱스 없이도 작동함
    
    return query
        .snapshots()
        .map((snapshot) => snapshot.docs.map((doc) {
          final tx = TaxTransaction.fromFirestore(doc);
          return tx;
        }).where((tx) {
          if (userId == null) return true;
          return tx.requester == userId;
        }).toList());
  }
  ...
  // ✏️ 요청 수정 처리 (v52: 날짜 수정 추가)
  Future<void> updateRequest(String approvalId, int newAmount, String newDescription, DateTime newDate) async {
    await _db.collection('approvals').doc(approvalId).update({
      'amount': newAmount,
