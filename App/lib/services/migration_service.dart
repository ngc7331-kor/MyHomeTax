...
2026-03-17 15:00:00,cw,회비 납부,-5000,-3000,3월 회비,2026-03-17,"cw: 5,000원, dk: 3,000원" """;
  ...
  // 🚀 마이그레이션 실행 함수 (v11: 초정밀 청크 배칭 최적화 버전)
  static Future<void> runMigration() async {
    final prefs = await SharedPreferences.getInstance();
    
    // v11 강제 재실행 여부 체크 (2025년 누락 데이터 복구 v11)
    if (prefs.getBool('is_migrated_v11') ?? false) {
      debugPrint("✅ 이미 데이터 11차 정밀 복구(v11)가 완료되었습니다.");
      return;
    }

    debugPrint("🚀 [v11] 2025년 누락 데이터 포함 초정밀 청크 마이그레이션 시작...");

    try {
      // 1. 기존 데이터 삭제 대상 조회
      final cwHistorySnap = await _db.collection('taxes').doc('cw').collection('history').get();
      final dkHistorySnap = await _db.collection('taxes').doc('dk').collection('history').get();
      final approvalsSnap = await _db.collection('approvals').get();

      // 2. 전체 데이터 작업을 일괄 큐에 수집 (Batch Chunking 적용)
      final List<void Function(WriteBatch)> operations = [];

      // 2-1. 기존 오염/중복 데이터 삭제 작업 추가
      for (var doc in cwHistorySnap.docs) {
        operations.add((batch) => batch.delete(doc.reference));
      }
      for (var doc in dkHistorySnap.docs) {
        operations.add((batch) => batch.delete(doc.reference));
      }
      for (var doc in approvalsSnap.docs) {
        operations.add((batch) => batch.delete(doc.reference));
      }

      // 2-2. 2024~2026 CSV 본문 파싱 및 복구 등록 작업 추가
      int cwTotal = 0;
      int dkTotal = 0;
      ...
      for (var line in csvLines) {
        ...
        if (cwVal != 0) {
          ...
          operations.add((batch) => batch.set(docRef, {
            'requester': 'cw',
      ...
      // 2-3. 최종 요약 통계 문서 등록 작업 추가
      final cwFinalRefund = (cwTotal * 0.3).round();
      final dkFinalRefund = (dkTotal * 0.3).round();

      final cwTaxRef = _db.collection('taxes').doc('cw');
      operations.add((batch) => batch.set(cwTaxRef, {
        'name': '채원',
        'totalTax': cwTotal,
        'totalRefund': cwFinalRefund,
        'lastUpdated': FieldValue.serverTimestamp(),
        'is_migrated_v11': true,
      }, SetOptions(merge: true)));

      final dkTaxRef = _db.collection('taxes').doc('dk');
      operations.add((batch) => batch.set(dkTaxRef, {
        'name': '도권',
        'totalTax': dkTotal,
      ...
      // 2-4. dataApprovals 대기 내역 복구 작업 추가
      ...
      for (var line in approvalLines) {
        ...
        final isUsage = actionType == "세금 사용" || actionType == "회비 납부" || cwVal < 0 || dkVal < 0;

        final approvalRef = _db.collection('approvals').doc();
        operations.add((batch) => batch.set(approvalRef, {
          'requester': requester,
          'description': memo.isEmpty ? '승인 대기' : memo,
          'amount': (cwVal == 0 ? dkVal : cwVal).abs(),
          'type': isUsage ? 'usage' : 'payment',
          'status': 'pending',
      ...
      // 3. 200개 크기의 청크(Chunk) 단위로 쪼개어 초고속 트랜잭션 커밋 실행
      debugPrint("📦 총 ${operations.length}개의 마이그레이션 작업이 대기 중입니다.");
      ...
      for (int i = 0; i < operations.length; i += chunkSize) {
        ...
        await batch.commit();
        debugPrint("✅ [v11] 청크 마이그레이션 완료: ${i + chunk.length} / ${operations.length}");
      }

      // 4. 전체 성공 시 SharedPreferences 플래그 설정 및 로그 출력
      await prefs.setBool('is_migrated_v11', true);
      debugPrint("🎉 [v11] 데이터 마이그레이션 및 정밀 복구 최종 성공!");
    } catch (e) {
      debugPrint("❌ [v11] 마이그레이션 에러: $e");
    }
  }
